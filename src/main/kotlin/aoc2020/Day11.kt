...
class Day11 {
    ...
    private data class SeatMap(val width: Int, val height: Int, val seats: MutableMap<Vec, Seat> = HashMap())
    ...
    private fun parse(src: File) : SeatMap {
        ...
        for ((y, line) in lines.withIndex()) {
            for ((x, c) in line.withIndex()) {
                if (c == 'L') plan.seats[Vec(x, y)] = Seat.EMPTY
            }
        }
        return plan
    }

    private fun adjacentPoints(p: Vec) : List<Vec> {
        return Dir8.values().map { p + it.delta }
    }

    // Check seats in 8 adjacent cells
    private fun seatTickAdjacent(pos: Vec, seat: Seat, map: SeatMap) : Seat {
    ...
    private fun look(map: SeatMap, pos: Vec, delta: Vec) : Seat? {
    ...
    private fun seatTickBeam(pos: Vec, seat: Seat, map: SeatMap) : Seat {
    ...
    private fun seatMapTick(map: SeatMap, seatTickFunc: (a: Vec, b: Seat, c: SeatMap) -> Seat) : SeatMap {
        val next = SeatMap(map.width, map.height)
        map.seats.entries.forEach { next.seats[it.key] = seatTickFunc(it.key, it.value, map) }
        return next
    }

    private fun solve(seatMap: SeatMap, seatTickFunc: (a: Vec, b: Seat, c: SeatMap) -> Seat) : Int {
